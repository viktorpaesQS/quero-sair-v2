...
type Notify = { plate: string; lat?: number; lng?: number; note?: string; ts: string };
const activeNotifications = new Map<string, Notify>(); // key: plate
...
app.post("/api/v1/parking/notify", (req, res) => {
  ...
  const data: Notify = { ...parsed.data, ts: new Date().toISOString() };
...
app.get("/api/v1/qrcode/:plate", async (req, res) => {
  ...
  if (!plate) return res.status(400).send("plate required");
